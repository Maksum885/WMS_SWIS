...
<body>
    ...
    <header>
        <div class="letterhead">
            ...
            <div style="display:table-cell; width:40%;" class="letterhead-doc-no">
                DO No.<br><b>{{ $deliveryOrder->do_no }}</b>
            </div>
        </div>
    </header>
    ...
    <div class="doc-date">Document Date: {{ $deliveryOrder->created_at->format('d M Y') }}</div>

    <div class="info-box">
        <div class="info-col">
            <div class="info-row"><div class="info-label">Sales Order</div><div class="info-value">{{ $deliveryOrder->salesOrder->so_no ?? '—' }}</div></div>
            <div class="info-row"><div class="info-label">Customer</div><div class="info-value">{{ $deliveryOrder->salesOrder->customer->name ?? '—' }}</div></div>
            <div class="info-row"><div class="info-label">Source Picking</div><div class="info-value">{{ $deliveryOrder->picking->picking_no ?? '— (manual entry)' }}</div></div>
            <div class="info-row"><div class="info-label">Status</div><div class="info-value"><span class="status-badge status-{{ $deliveryOrder->status }}">{{ ucfirst($deliveryOrder->status) }}</span></div></div>
        </div>
        <div class="info-col">
            <div class="info-row"><div class="info-label">Delivery Date</div><div class="info-value">{{ optional($deliveryOrder->delivery_date)->format('d M Y') ?: '—' }}</div></div>
            <div class="info-row"><div class="info-label">Vehicle No</div><div class="info-value">{{ $deliveryOrder->vehicle_no ?: '—' }}</div></div>
            <div class="info-row"><div class="info-label">Driver</div><div class="info-value">{{ $deliveryOrder->driver_name ?: '—' }}</div></div>
            <div class="info-row"><div class="info-label">Remark</div><div class="info-value">{{ $deliveryOrder->remark ?: '—' }}</div></div>
        </div>
    </div>
        ...
        <tbody>
            @forelse ($deliveryOrder->lines as $l)
                <tr>
                    ...
                    <td class="ta-right">{{ fmt_qty($l->qty_delivered) }}</td>
                    <td>{{ $l->uom }}</td>
                    <td>{{ $l->lot_no ?: '—' }}</td>
                </tr>
            @empty
    ...
    <div class="summary">Total line items: {{ $deliveryOrder->lines->count() }}</div>

    <div class="sign-box">
        <div class="sign-col">
            <div class="sign-label">Created By</div>
            <div class="sign-line"></div>
        </div>
        <div class="sign-col">
            <div class="sign-label">Acknowledged By (Receiver)</div>
            <div class="sign-line"></div>
        </div>
    </div>
</body>
